...
export const LoginPage: React.FC = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/too-many-requests') setError('Too many attempts. Try again later.');
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/invalid-verification-code') setError('Wrong OTP. Please check and retry.');
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/60 border border-slate-200">
          ...
          {method === 'google' && (
            <div className="space-y-4">
              <p className="text-xs text-slate-500">Sign in securely using your Google account.</p>
              <button
                type="button"
          ...
          {method === 'email' && (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              ...
              {error && <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs font-semibold">{error}</div>}
              <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-gradient-to-r from-brand-600 to-amber-600 hover:from-brand-700 hover:to-amber-700 text-white font-bold text-sm shadow-md shadow-brand-600/25 flex items-center justify-center gap-2 transition-all disabled:opacity-50">
            ...
            <div className="space-y-4">
              {phoneStage === 'INPUT' ? (
                <form onSubmit={handleSendOtp} className="space-y-4">
                  <p className="text-xs text-slate-500">We'll send a 6-digit OTP to your mobile via SMS.</p>
                  <div>
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <button type="button" onClick={() => { setPhoneStage('INPUT'); setOtp(''); setError(null); }} className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-800 mb-1 transition-colors">
                  ...
                  <div className="text-center">
                    <button type="button" onClick={handleResend} disabled={resendCooldown > 0} className="text-xs font-bold text-brand-600 hover:text-brand-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
          ...
          <div className="pt-5 mt-5 border-t border-slate-100 text-center">
            <p className="text-xs text-slate-500">
              ...
              <Link to="/register" className="font-bold text-brand-600 hover:underline">Register here</Link>
            </p>
          </div>
        </div>
